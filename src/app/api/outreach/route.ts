...
import { assertUuid, assertNumber } from '@/lib/validate';
...
export async function POST(req: NextRequest) {
  try {
    const limit = rateLimit(getClientIp(req), { windowMs: 60_000, maxRequests: 20 });
    if (!limit.allowed) {
      return NextResponse.json({ error: 'Rate limit exceeded' }, { status: 429 });
    }

    const { leadId, stage, overrideEmail } = await req.json();
    const leadIdValid = assertUuid(leadId, 'leadId');
    const stageValid = assertNumber(stage, 'stage');
    if (stageValid < 1 || stageValid > 3) {
      return NextResponse.json({ error: 'stage must be 1, 2, or 3' }, { status: 400 });
    }

    const lead = await db.select().from(schema.leads).where(eq(schema.leads.id, leadIdValid)).get();
    if (!lead) return NextResponse.json({ error: 'Lead not found' }, { status: 404 });

    const site = await db.select().from(schema.sites).where(eq(schema.sites.leadId, leadIdValid)).get();
    ...
    await db.update(schema.leads).set({ status: 'contacted' }).where(eq(schema.leads.id, leadIdValid)).run();
    logger.info('Outreach sent', { leadId: leadIdValid, stage: stageValid, toEmail });
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const stage = typeof body.stage === 'number' && body.stage >= 1 && body.stage <= 3 ? body.stage : 1;
    ...
    for (const lead of eligibleLeads) {
      ...
      try {
        const { subject, body: emailBody } = await sendOutreachEmail({
          to: lead.email,
          businessName: lead.businessName,
          previewUrl,
          senderName: process.env.SENDER_NAME ?? 'Your Name',
        ...
        logger.warn('Bulk outreach failed for lead', { leadId: lead.id, error: String(err) });
        skipped++;
      }
    }

    logger.info('Bulk outreach complete', { sent, skipped, stage });
